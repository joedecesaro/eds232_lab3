...
1. `r mean(repr_data$relative_held)`
2. `r sum(repr_data$relative_held < 0.1)`
3. `r sum(repr_data$relative_held < 0.2)`
4. Generally I would say that there is no relationship that is highly evident.
```{r}
...
Its worth noting that prioritizr is a decision support tool. This means that it is designed to help you make decisions---it can't make decisions for you.
...
1. 15 units and therefore 2.9% of all units
2. There are 4 units closely distributed in the northeast/east. More than half of the prioritized units are in the eastern portion of the area.
3. Yes, we can verify that all of the targets were met based on the code chunk below.

```{r}
s1_sum <- eval_feature_representation_summary(p1, s1[, "solution_1"])
min_s1 <- (min(s1_sum$relative_held))
min_s1
```
...
2.
```{r}
eval_n_summary(p2, s2[, "solution_1"])$cost
eval_n_summary(p3, s3[, "solution_1"])$cost
eval_n_summary(p4, s4[, "solution_1"])$cost
```

3. The solutions with more planning units do have a greater cost, because you need to buy more units to meet the larger targets and by excluding the cheap "locked out" land you need to buy a few more units.

4. Some of the land to reach the goals of s1 are located in the "locked-in" land and this land can't be purchased in s2. To reach the same target more expensive land is likely required.

5. By excluding the cheaper degraded areas you need to buy a few more units to reach the 10% target.
...
1. See costs below. s5 likely costs more than s4 because some the land that has been grouped together is more expensive than the disjointed land proposed in s4.
```{r}
eval_cost_summary(p4, s4[, "solution_1"])$cost
eval_cost_summary(p5, s5[, "solution_1"])$cost
```

2. This penalty makes the costs of the solutions roughly the same and their maps are mostly similar with a few minor differences because the boundary penalty is so low so it makes sense that they have relatively the same costs. See code below for visualization and cost of s6.
```{r, out.width = "65%"}
# make prioritization problem
...
3. This solution is much more expensive because the large penalty on boundary length requires everything to be mostly under one large area as seen in the map below. This large area is likely driving up the costs.
```{r}
# make prioritization problem
